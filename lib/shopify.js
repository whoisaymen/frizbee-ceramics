  ...
  const query = `
  {
    product(handle: "${handle}") {
      ...
      metafields(identifiers: [
        ...
        {namespace: "custom", key: "detail_page_gallary"}
      ]) {
        id
        namespace
        key
...
export async function getPreOrderProductIds() {
  const query = `{
    products(first: 250) {
      edges {
        node {
  ...
  const preOrderIds = edges.reduce((acc, edge) => {
    const node = edge.node;
    const variant = node.variants.edges[0]?.node;

    // Pre-order condition
    if (node.availableForSale && (variant?.quantityAvailable ?? 0) <= 0) {
      const numericId = extractNumericId(node.id);
      if (!isNaN(numericId)) acc.push(numericId);
    }

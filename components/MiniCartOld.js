...
export default function MiniCart({ cart }) {
  ...
  const isMobile2 = useMediaQuery({ query: "(max-width: 768px)" });
  ...
  const cartButtonStyle2 = {
    position: "fixed",
    right: "10px", // Adjust the left positioning as needed
    bottom: "24px", // Slide up if cart is open
    zIndex: 9,
    transition: "bottom 0.5s ease-in-out", // Adjust transition for sliding up
    backgroundColor: cartQuantity > 0 ? "#F7D949" : "white",
    color: "black",
  ...
  return (
    <>
      <button
        style={cartButtonStyle2}
        className="text-sm font-light tracking-wide py-4 lg:hidden"
        onClick={() => setCartOpen(!cartOpen)}
      >
        CART({cartQuantity})
      </button>
      <div
        className="!hidden !lg:flex text-sm font-light lg:text-base"
      ...
      <Transition.Root show={cartOpen} as={Fragment}>
        ...
        >
          <div className="absolute inset-0 overflow-hidden">
            ...
            <div
              className={`fixed inset-y-0 border-red-500 border-3 right-0 md:top-0 flex w-full md:max-w-[34%] lg:max-w-[calc(25%+1px)] items-end md:items-start h-full`}
                ...
                <div className="w-full border-black lg:border-l border-t mt-4">
                  <div className="flex flex-col h-[75vh] lg:h-[calc(100vh-46px)] overflow-y-scroll bg-white shadow-xl relative">
                            ...
                            >
                              {cart.map((product) => (
                                ...
                                >
                                  ...
                                  ></div>
                                  <div className="relative flex-shrink-0 w-[33%] h-[15vh] overflow-hidden border-gray-400 border-r-0">
                                    <Image
                                      src={product.image}
                                      alt={product.title}
                                      width={500}
                    ...
                    <div className="mt-0">
                      <a
                        href={checkoutUrl}
                        className={`uppercase tracking-tight flex items-center justify-center px-6 py-2 text-sm font-light text-white bg-[#000] border border-transparent shadow-sm hover:bg-gray-800 ${
